...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSLotusConnection) -> CoinState:
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            ...
            txs: List[TransactionRecord] = [dl_tx, lotus_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSLotusConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            lotus_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, lotus_tx.spend_bundle])
                ),
                dataclasses.replace(lotus_tx, spend_bundle=None),
            ]
