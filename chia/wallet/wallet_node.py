...
class WalletNode:
    ...
    _server: Optional[LotusServer] = None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSLotusConnection] = None
    ...
    def on_disconnect(self, peer: WSLotusConnection):
    ...
    async def is_peer_synced(
        self, peer: WSLotusConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSLotusConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSLotusConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSLotusConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSLotusConnection]:
        """
        ...
        synced_and_trusted: List[WSLotusConnection] = []
        synced: List[WSLotusConnection] = []
        trusted: List[WSLotusConnection] = []
        neither: List[WSLotusConnection] = []
        all_nodes: List[WSLotusConnection] = self.server.get_full_node_connections().copy()
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSLotusConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSLotusConnection):
        if self._wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSLotusConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSLotusConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSLotusConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSLotusConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSLotusConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSLotusConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
