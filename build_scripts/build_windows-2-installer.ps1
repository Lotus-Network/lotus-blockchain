...
Write-Output "   ---"
Write-Output "Copy chia executables to lotus-blockchain-gui\"
Write-Output "   ---"
...
Write-Output "   ---"
Copy-Item ".\dist\win-unpacked" -Destination "$env:GITHUB_WORKSPACE\lotus-blockchain-gui\Chia-win32-x64" -Recurse
mkdir "$env:GITHUB_WORKSPACE\lotus-blockchain-gui\release-builds\windows-installer" -ea 0
Copy-Item ".\dist\ChiaSetup-$packageVersion.exe" -Destination "$env:GITHUB_WORKSPACE\lotus-blockchain-gui\release-builds\windows-installer"
