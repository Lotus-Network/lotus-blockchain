...
Set-Location -Path "lotus-blockchain-gui\packages\gui" -PassThru
...
electron-builder build --win --x64 --config.productName="Lotus"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe verify /v /pa .\dist\LotusSetup-$packageVersion.exe
   }   Else    {
...
Write-Output "   ---"
Copy-Item ".\dist\win-unpacked" -Destination "$env:GITHUB_WORKSPACE\lotus-blockchain-gui\Lotus-win32-x64" -Recurse
mkdir "$env:GITHUB_WORKSPACE\lotus-blockchain-gui\release-builds\windows-installer" -ea 0
Copy-Item ".\dist\LotusSetup-$packageVersion.exe" -Destination "$env:GITHUB_WORKSPACE\lotus-blockchain-gui\release-builds\windows-installer"
