...
"""
run_block: Convert an encoded FullBlock from the Lotus blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../lotus/types/full_block.py) in json format
...
$ca_root is the directory containing your current Lotus config files
$hash is the header_hash of the [BlockRecord](../lotus/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../lotus/types/condition_opcodes.py)
...
from lotus.full_node.generator import create_generator_args
...
from lotus.wallet.cat_wallet.cat_utils import match_cat_puzzle
