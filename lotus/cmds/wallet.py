...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much lotus to send, in LCH", type=str, required=True)
@click.option("-e", "--memo", help="Additional memo for the transaction", type=str, default=None)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in LCH",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the LCH", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min_coin_amount",
    help="Ignore coins worth less then this much LCH or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@wallet_cmd.command("make_offer", short_help="Create an offer of LCH/CATs for LCH/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in LCH.",
    type=str,
    default="0",
    show_default=True,
