...
class CATWallet:
    ...
    async def create_tandem_lch_tx(
        ...
        if fee > amount_to_claim:
            lotus_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(lotus_coins)[0].name()
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some LCH as part of this
        ...
        need_lotus_transaction = (fee > 0 or regular_lotus_to_claim > 0) and (fee - regular_lotus_to_claim != 0)
            ...
            if first:
                ...
                if need_lotus_transaction:
                    if fee > regular_lotus_to_claim:
                        lotus_tx, _ = await self.create_tandem_lch_tx(
                            fee,
                            uint64(regular_lotus_to_claim),
                        ...
                        lotus_tx, _ = await self.create_tandem_lch_tx(
                            fee, uint64(regular_lotus_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        if lotus_tx is not None and lotus_tx.spend_bundle is not None:
            lotus_spend_bundle = lotus_tx.spend_bundle
