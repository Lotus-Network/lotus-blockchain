...
from lotus.util.config import create_default_lotus_config, load_config, save_config
...
def create_config(
    ...
    create_default_lotus_config(lotus_root)
    create_all_ssl(
        lotus_root,
    ...
    # load config
    config = load_config(lotus_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(fingerprint), "tlch")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(lotus_root, "config.yaml", config)
    return config


async def start_simulator(lotus_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=lotus_root)
    await service.start()
...
async def get_full_lotus_simulator(
    automated_testing: bool = False, lotus_root: Optional[Path] = None, config: Optional[Dict[str, Any]] = None
    ...
    if lotus_root is None:
        lotus_root = Path(tempfile.TemporaryDirectory().name)
    ...
    crt_path = lotus_root / config["daemon_ssl"]["private_crt"]
    key_path = lotus_root / config["daemon_ssl"]["private_key"]
    ca_crt_path = lotus_root / config["private_ssl_ca"]["crt"]
    ca_key_path = lotus_root / config["private_ssl_ca"]["key"]
    with Lockfile.create(daemon_launch_lock_path(lotus_root)):
        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(lotus_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.start()  # type: ignore[no-untyped-call]

        async for simulator in start_simulator(lotus_root, automated_testing):
            yield simulator, lotus_root, config, mnemonic, fingerprint
