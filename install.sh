      ...
      if [ "$BEST_VERSION" = "" ]; then
        ...
        if [ "$BEST_VERSION" = "3" ]; then
          ...
          if [[ "$PY3_VERSION" =~ 3.11.* ]]; then
            echo "Lotus requires Python version < 3.11.0" >&2
...
python -m pip install --extra-index-url https://pypi.lotus.net/simple/ miniupnpc==2.2.2
python -m pip install -e ."${EXTRAS}" --extra-index-url https://pypi.lotus.net/simple/
...
echo "https://keybase.io/team/lotus_network.public"
echo ""
