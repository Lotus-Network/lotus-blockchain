...
## 1.5.1 Lotus blockchain 2022-8-23
...
- Add `lotus db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Removing `logging.basicConfig()` from `LotusServer.__init__()`
...
- Log `lotus_full_version_str` in daemon and all services
...
- Substantially reduced overall size of Lotus packages
...
- Make sure lotus commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 Lotus blockchain 2022-7-26
...
- Update `lotus wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the Lotus blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the Lotus blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 Lotus blockchain 2022-6-29

### Added

- Added support for NFTs!!! :party:
- Added `lotus wallet nft` command (see <https://docs.lotusgang.org/docs/13cli/did_cli>)
- Added `lotus wallet did` command (see <https://docs.lotusgang.org/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.lotusgang.org/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.lotusgang.org/docs/12rpcs/nft_rpcs>)
...
- Added CLI options `--sort-by-height` and –sort-by-relevance` to `lotus wallet get_transactions`
...
- New coin selection algorithm based on bitcoin knapsack. Previously lotus selected the largest coin
...
- Updated warning about `LOTUS_ROOT` being set when running init
...
## 1.3.5 Lotus blockchain 2022-5-11
...
## 1.3.4 Lotus blockchain 2022-4-19
...
- Added `-n`/`--new-address` option to `lotus wallet get_address`
...
- Fixed typo in command line argument parsing for `lotus db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `lotus configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `lotus db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `lotus show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `lotus wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `lotus wallet show`
...
- You cannot install and run lotus blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 Lotus blockchain 2022-4-02
...
- Fixed exception when `lotus keys migrate` is run without needing migration.
...
- Added checks to ensure wallet address prefixes are either `lch` or `tlch`.
...
## 1.3.0 Lotus blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of Lotus (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Lotus will not use when launching process pools. Using 0 will allow Lotus to use all cores for process pools. Set the default to 0 to allow Lotus to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, lotus keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using lotus keys derive search.
...
- CLI option lotus show will display the currently selected network (mainnet or testnet).
- CLI option lotus plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with lotus show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the lotus-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI lotus show -c when No Info should be displayed.
...
- When you close the Lotus app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the Lotus and relaunch the Lotus app.
...
## 1.2.11 Lotus blockchain 2021-11-4

### Added

- Farmers rejoice: today's release integrates two plotters in broad use in the Lotus community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/Chia-Network/chia-blockchain/wiki/Alternative--Plotters).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Lotus Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `lotus init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `lotus plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 Lotus blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/Chia-Network/chia-blockchain/wiki/Passphrase-Protected-Lotus-Keys-and-Key-Storage-Migration). We also launched a new Chialisp compiler in clvm_tools_rs which substantially improves compile time for Chialisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/Chia-Network/chia-blockchain/wiki/Passphrase-Protected-Lotus-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chialisp compiler in clvm_tools_rs in lotus-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other lotus services.
...
- Updated the old comment on lotus/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Lotus DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Lotus blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Lotus blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Lotus's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://lotusgang.org/download/>.
...
- We have made many full node changes to support our upcoming Lotus Asset Token (CAT) standard and our upcoming standalone light wallet, which will use Lotus's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming Lotus Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install lotus-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the lotus install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to lotus wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Lotus's service ip.lotusgang.org.
...
## 1.2.3 Lotus blockchain 2021-07-26
...
- Thanks @mishan for your change to 'lotus plotnft show' to display Percent Successful Points.
...
## 1.2.2 Lotus blockchain 2021-07-13
...
## 1.2.1 Lotus blockchain 2021-07-12
...
## 1.2.0 Lotus blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `lotus plots create`) using code from before June 25th are invalid on mainnet.
...
- `lotus farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `lotus configure` now supports command line updates to peer count and target peer count.
...
- All Lotus repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `lotus plotnft` to manage pools.
...
- Correctly display private key in `lotus keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `lotus keys show`.
...
- Thank you to @LotusMineJP for various improvements.
...
- `lotus farm summary` better handles wallet errors.
...
- Mainnet uses port 5980 and other constants and service names were changed for mainnet.
...
- This is a hard fork/breaking change from RC6/7. TLCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.lotusgang.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `lotus farm summary` will now display TLCH or LCH as appropriate.
...
- We have moved to a new and much higher scalability download.lotusgang.org to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.lotusgang.org/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.lotusgang.org/latest/Setup-MacOS.dmg).
...
- The Farm page will now no longer get stuck at 50 TLCH farmed.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `lch_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TLCH will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.lotusgang.org/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.lotusgang.org via https, for example.
...
- RC3 is a new chain to support the last major chialisp changes. TLCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'lotus wallet send' now takes in TLCH or LCH as units instead of mojos.
...
- The Beta 27 chain is a hard fork. All TLCH from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TLCH/LCH per block for a total of 9216 TLCH per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from lotus-blockchain, [lotus-blockchain-gui](https://github.com/Chia-Network/chia-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `lotus-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `lotus wallet`, was incorrectly displaying TLCH balances as if they were Coloured Coins.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TLCH. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TLCH compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TLCH.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [lotusgang.org](https://lotusgang.org/). Among the improvements this gives the Lotus blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TLCH from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TLCH to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TLCH to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TLCH you've won so far. Harvester and Full Node connections have moved to Advanced Options.
...
- Testnet is now running at the primary port of 55980. Update your routers appropriately. This opens 5980 for mainnet.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Chia-Network/chia-blockchain/tree/dev/electron-react/src/locales) of the lotus-blockchain repository.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Lotus Network at backup.lotusgang.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Lotus backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.lotusgang.org, your own installation, or a third party's version of it.
...
- Fixed a typo in `lotus show -w` with thanks to @pyl on Keybase.
...
- Changed LCH units to TLCH units for testnet.
...
other chains.
- Keys are now derived with the EIP 2334 (m/12381/5980/a/b).
...
- The user interface now refers to lotus coins with their correct currency code of LCH.
...
- There is a new command `lotus init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `lotus-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chia-Network/chia-blockchain/wiki/Updating-beta-software). If you've set `$LOTUS_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- There is now full transaction support on the Lotus blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Chia-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the lotus management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `lotus-start-farmer &`. The [README.md](https://github.com/Chia-Network/chia-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- The libraries chia-pos, lotus-fast-vdf, and lotus-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Chia-Network/chiapos), [chiavdf](https://github.com/Chia-Network/chiavdf), and [chaibip158](https://github.com/Chia-Network/chiabip158). They are brought in by lotus-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Chia-Network/bls-signatures).
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 5980 or their UPnP port.
...
- More details on the release at [https://www.lotusgang.org/developer/](https://www.lotusgang.org/developer/)
