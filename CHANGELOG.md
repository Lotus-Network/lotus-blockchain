...
- Added `lotus wallet nft` command (see <https://docs.lotus.net/docs/13cli/did_cli>)
- Added `lotus wallet did` command (see <https://docs.lotus.net/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.lotus.net/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.lotus.net/docs/12rpcs/nft_rpcs>)
...
- Beta builds are built on every merge to main, and are now available from <https://lotus.net/download/>.
...
- Switched IP address lookup to first use Lotus's service ip.lotus.net.
...
- This is a hard fork/breaking change from RC6/7. TLCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.lotus.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.lotus.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.lotus.net via https, for example.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [lotus.net](https://lotus.net/). Among the improvements this gives the Lotus blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Lotus Network at backup.lotus.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Lotus backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.lotus.net, your own installation, or a third party's version of it.
...
- More details on the release at [https://www.lotus.net/developer/](https://www.lotus.net/developer/)
