...
from lotus.clvm.spend_sim import SimClient, SpendSim
...
from lotus.util.config import create_default_lotus_config, lock_and_load_config
...
from lotus.simulator.block_tools import BlockTools, test_constants, create_block_tools, create_block_tools_async
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the lotus root directory
...
@pytest.fixture(scope="function")
def tmp_lotus_root(tmp_path):
    """
    Create a temp directory and populate it with an empty lotus_root directory.
    """
    path: Path = tmp_path / "lotus_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_lotus_root) -> Path:
    """
    Create a temp lotus_root directory and populate it with a default config.yaml.
    Returns the lotus_root path.
    """
    root_path: Path = tmp_lotus_root
