...
from lotus.rpc.farmer_rpc_api import (
    FilterItem,
...
from lotus.simulator.block_tools import get_plot_dir
from lotus.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "lch"), encode_puzzle_hash(new_ph_2, "lch"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "lch"), encode_puzzle_hash(observer_pool, "lch")
    )
    ...
    assert config["farmer"]["lch_target_address"] == encode_puzzle_hash(observer_farmer, "lch")
    assert config["pool"]["lch_target_address"] == encode_puzzle_hash(observer_pool, "lch")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "lch")
