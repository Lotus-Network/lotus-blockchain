...
from lotus.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
...
from lotus.wallet.derive_keys import find_authentication_sk, find_owner_sk
from lotus.wallet.transaction_record import TransactionRecord
from lotus.wallet.util.wallet_types import WalletType
from lotus.simulator.block_tools import BlockTools, get_plot_dir
...
class TestPoolWalletRpc:
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc, trusted_and_fee, self_hostname):
        ...
        async with TemporaryPoolPlot(bt, status.p2_singleton_puzzle_hash) as pool_plot:
            ...
            tr: TransactionRecord = await client.send_transaction(
                1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "tlch")
            )
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc, trusted_and_fee, self_hostname):
        ...
        async with TemporaryPoolPlot(bt, status.p2_singleton_puzzle_hash) as pool_plot:
            ...
            # Balance stars at 6 LCH and 5 more blocks are farmed, total 22 LCH
    ...
    async def test_leave_pool(self, setup, trusted_and_fee, self_hostname):
        ...
        try:
            ...
            await time_out_assert(timeout=MAX_WAIT_SECS, function=have_lotus)
