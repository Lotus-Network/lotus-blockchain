...
Contact any of the team members on [Keybase](https://keybase.io/team/lotus_network.public), which we use as the main communication method. You can also comment on any Trello card.
...
In the event an emergency fix is required for the release version of Lotus, members of the Lotus organization will create a feature branch from the current release branch `latest`.
...
sh install.sh -d
. ./activate
black . && isort benchmarks build_scripts lotus tests tools *.py && mypy && flake8 benchmarks build_scripts lotus tests tools *.py && pylint benchmarks build_scripts lotus tests tools *.py
...
The current official testnet is testnet10. Look at `lotus/util/initial_config.yaml` to see the configuration parameters
...
If you are not a Lotus organization member, you can enquire about deploying a `beta testnet` in the public dev Keybase channel.
